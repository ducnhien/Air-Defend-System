...
class PanTiltTracker:
    ...
    def update(self, cx, cy, frame_w=960, frame_h=540):
        ...
        # Cho phép dải góc quét chạy rộng (-10 đến 200)
...
system_state = {"distance": 8190, "status": "IDLE", "pan": 90, "tilt": 90, "label": "-", "log_msg": "",
                "is_alarm": False, "fps": 0}  # Khởi tạo thêm key fps


def generate_frames():
    global system_state
    prev_frame_time = 0  # Biến lưu mốc thời gian của khung hình trước
    ...
    while True:
        ...
        # ==============================================================================
        # 🚨 ĐOẠN ĐIỀU KIỆN KIỂM TRA KHOẢNG CÁCH 5000mm HOẶC CÓ CHUYỂN ĐỘNG
        # ==============================================================================
        if distance <= 5000 or motion == 1:
        ...
        system_state = {
            ...
            "log_msg": log_msg,
            "is_alarm": is_alarm,
            "fps": int(fps)  # Đưa dữ liệu FPS vào state gửi lên Web
        }
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        /* Thay đổi grid thành 3 cột để thêm ô FPS nhìn cho cân đối */
        .bottom-dashboard { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; }
        .telemetry-card { background: rgba(3, 20, 34, 0.7); border: 1px solid #002d3d; border-left: 4px solid #00f0ff; padding: 10px 12px; }
        /* Riêng ô FPS cho một màu sắc đặc trưng (ví dụ: xanh lá mạ của hệ thống cyber) */
        .telemetry-card.fps-card { border-left: 4px solid #a6ff00; }
        ...
        .fps-card .card-data span { color: #a6ff00; }
...
<body>
    <div class="main-container">
        ...
        <div class="radar-right-panel">
            ...
            <div class="bottom-dashboard">
                ...
                <div class="telemetry-card fps-card">
                    <div class="card-label">TỐC ĐỘ XỬ LÝ</div>
                    <div class="card-data" id="lbl-fps">0<span>FPS</span></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        sse.onmessage = function(e) {
            ...
            // 🟢 Cập nhật giá trị FPS vào giao diện từ SSE dữ liệu thực tế
            document.getElementById("lbl-fps").innerHTML = `${data.fps}<span>FPS</span>`;
